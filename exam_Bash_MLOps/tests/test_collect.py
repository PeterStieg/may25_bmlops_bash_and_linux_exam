...
def test_sales_csv_structure():
    ...
    with open(log_file_path, "a") as f, redirect_stdout(f):
        ...
        try:
            ...
            assert (
                len(df.columns) == 3
            ), f"The CSV must contain exactly 3 columns, found {len(df.columns)}"
            assert "sales" in df.columns, "The CSV must contain a 'sales' column"
            assert (
                not df["sales"].isnull().any()
            ), "The 'sales' column must not contain any NaN values"
            assert np.all(
                np.equal(np.floor(df["sales"]), df["sales"])
            ), "The 'sales' column must contain only integers"
            assert (
                df["sales"] >= 0
            ).all(), "The 'sales' column must contain only positive values"
